          ...
          {props.messages.map((message, index) => {
            ...
            <div key={index} className={`flex ${message.senderId._id === props.userId ? 'justify-end' : 'justify-start'}`}>
              ...
              message.fileType.startsWith("image/") ? (
                <div key={index} className={`flex ${message.senderId._id === props.userId ? 'justify-end' : 'justify-start'}`}>
                  <div className={`p-1 rounded mb-1 max-w-sm break-words ${message.senderId._id === props.userId ? 'bg-[#005c4b]' : 'bg-gray-500'}`}>
                ...
                <div key={index} className={`flex ${message.senderId._id === props.userId ? 'justify-end' : 'justify-start'}`}>
                  <div className={`p-1 rounded mb-1 ${message.senderId._id === props.userId ? 'bg-[#005c4b]' : 'bg-gray-500'}`}>
                  <div className='relative'>
                ...
                <div key={index} className={`flex ${message.senderId._id === props.userId ? 'justify-end' : 'justify-start'}`}>
                  <div className={`p-1 rounded mb-1 ${message.senderId._id === props.userId ? 'bg-[#005c4b]' : 'bg-gray-500'}`}>
                    ...
                    <img src={`https://res.cloudinary.com/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME}/image/upload/f_auto,q_auto/v1/${message.fileUrl}`}
                      ...
                      <div className={`h-16 rounded flex justify-between mx-auto p-3 ${message.senderId._id === props.userId ? 'bg-[#125347]' : 'bg-[#3b3f3f47]'}`}>
                        <div>
                      ...
                      <div className={`flex ${message.senderId._id === props.userId ? 'justify-end' : 'justify-start'}`}>
